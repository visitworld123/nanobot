"""Tests for nanobot.utils.helpers — path safety, truncation, decode."""
...
def test_safe_path_within_workspace():
    from nanobot.utils.helpers import safe_path, WORKSPACE_DIR
    result = safe_path("test.txt")
    assert str(result).startswith(str(WORKSPACE_DIR.resolve()))


def test_safe_path_escape_rejected():
    from nanobot.utils.helpers import safe_path
    with pytest.raises(ValueError, match="escapes workspace"):
        safe_path("../../etc/passwd")


def test_truncate_short():
    from nanobot.utils.helpers import truncate
    text = "hello"
...
def test_decode_output_utf8():
    from nanobot.utils.helpers import decode_output
